...
import com.kware.policy.service.vo.ClusterWorkloadPod;
import com.kware.policy.service.vo.PromMetricContainer;
...
//	private final static String JPATH_VALUE  = "$.value";
	...
	public void analyze(Integer _clUid, int _prqlUid, String _prqlResult) {
		...
		try {
			for (Map<String, Object> t : resultList) {
				/*
				String singleResult = JSONUtil.getJsonStringFromMap(t);
				t.clear();
	...
	/**
	 json데이터를 추출하기 위한 jsonpath가 다음처럼 정의되어 있음
		{
		  "key": "node_info",
		  "node": "$.metric.Hostname",
		  "instance": "$.metric.instance,$.metric.kubernetes_io_hostname",
		  "timestamp": "$.value[0]",
		  "gpu_temp_$.metric.gpu": "$.value[1]"
		}
	 * @param _prqlUid
	...
	private Map<String, Object> extract(Object _prqlSingleResult) {
		...
		try {
			...
			for(String key: jpathMap.keySet()) {
				...
				//{"requests_$.metric.resource": "$.metric.host_ip, $.metric.instance"} 중에서 key와 관련 부분
				...
				//배열로 변경: {"requests_$.metric.resource": "$.metric.host_ip, $.metric.instance"} 중에서 value와 관련 부분				
			...
			/* 테스트1
			 * jsonpath에서 조회한 결과는 모두 ObjectMapper를 통해 변환하던지, 아니면 개별적인 객체로 생성하는 방식으로 처리해야겠네.
			Map<String,Object> tempMap = ctx.read("$.metric");
            ...
            // resultNode를 깊은 복사
            JsonNode deepClonedResultNode = objectMapper.readTree(resultNode.toString());

            // 깊은 복사된 JSON 출력
            String clonedJsonString = objectMapper.writerWithDefaultPrettyPrinter().writeValueAsString(deepClonedResultNode);
            System.out.println("Cloned JSON String:");
			...
			*/
			...
			/* 테스트2 */ 
			 /*
			 Map<String,Object> tempMap = ctx.read("$.metric");
			 Map<String,Object> tMap = (Map)SerializationUtils.clone((HashMap)tempMap);
			 */
    ...
    /**
     * PromMetricNode, PromMetricPod, PromMetricContainer에 각 데이터를 매핑하는 함수
     * 이미 등록된 각 Object Method를 통해서 멤버 함수 호출을 통해 데이터를 설정한다. 
     * @param _metricObj
     * @param _valsMap
     * @return
     */
	private Object makePareData(Object _metricObj, Map<String, Object> _valsMap) {
		...
		_valsMap.forEach((key, value)->{
			if(parseLog.isInfoEnabled())
				parseLog.info("Mapping Target: {}  k,v:{},{}", value==null? null: value.getClass().getName(), key, Objects.toString(value, null));
